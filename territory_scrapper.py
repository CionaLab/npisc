...
def get_expression_info(
    ...
    """
    Retrieve the "Expression Territory" and "Predicted Gene" from the specified URL.
    ...
    Returns:
    - Tuple[List[str], Optional[str]]: A tuple containing a list of territories and the predicted gene or None if it's not reported.
    """
    ...
    # Extracting the Expression Territory information
    territories = tree.xpath(
        '//*[@id="informations"]/div[3]/div/div[3]/div/div/table/tr/td[1]/a/text()'
    )

    # Extracting the Predicted Gene information
    predicted_genes = tree.xpath(
        '//*[@id="informations"]/div[3]/div/div[2]/div[3]/p/a/text()'
    )
    predicted_gene = predicted_genes[0] if predicted_genes else None

    return territories, predicted_gene
...
def main():
    ...
    with args.input as input_file, args.output as output:
        ...
        # Writing the header
        writer.writerow(["URL", "Gene", "Territory"])

        for line in input_file:
            url = line.strip()
            territories, predicted_gene = get_expression_info(url, not args.insecure)

            for territory in territories:
                writer.writerow(
                    [url, predicted_gene if predicted_gene else "None", territory]
                )
